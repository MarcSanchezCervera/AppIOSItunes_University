...
/* VM encargar de manejar los vídeos recibidos de la API y pasarlos a la View correspondiente*/
...
class VideoStore{
    ...
    func fetchVideos(query: String) {
        ...
        Task {
            do{
                ...
                let urlString = "https://itunes.apple.com/search?term=\(trimmedQuery)&entity=musicVideo"
    ...
    func fetchVideosWithPreferences(query: String, country: String, explicit: Bool) {
        guard self.state != .loading else { return }
        self.state = APIState.loading
        ...
        Task {
            do{
                ...
                var urlString = "https://itunes.apple.com/search?term=\(trimmedQuery)&entity=musicVideo"
    ...
    // Arrays de opciones posibles para selección de campos
    let explicitOptions: [String] = [
        "Yes",
        "No"
    ]
